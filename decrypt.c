...
int findIndex(const char* key, char target) {
    for (int i = 0; i < 26; i++) {
        if (key[i] == target) {
            return i;
        }
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        {
            printf("Usage: %s [file to be decrypted] -p [key] \n", argv[0]);
    ...
    if (outputFile == NULL)
    {
        char* baseFileName = strtok(argv[1], ".");
        if (baseFileName == NULL) {
            baseFileName = argv[1];
        }

        sprintf(outputFileName, "%s_decrypted.txt", baseFileName);
    ...
    printf("The file gets decrypted !! \n");
    return 0;
}
